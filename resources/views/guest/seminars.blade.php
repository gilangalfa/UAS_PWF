...
<body class="flex flex-col h-full m-0 ">
    ...
    <header >
            <x-header title="Seminar"></x-header>    
    </header>
        ...
        <main class="flex justify-center p-8 gap-8 flex-wrap bg-white w-full ">    
            ...
            @foreach ($seminars as $seminar)
                <div class="max-w-2xl min-h-fit bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div class="p-5">
                        <a href="#">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $seminar->name }}</h5>
                        </a>
                        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ $seminar->description }}</p>
